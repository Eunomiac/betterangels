...
const _parseSearchFunc = (obj: object, searchFunc: object | number | string | ((a: string | number) => boolean)): ((a: string | number) => boolean) => {
  ...
  if (typeof searchFunc === "function") { 
    return searchFunc as ((a: string | number) => boolean);
  }
  if (typeof )

  if (["list", "array"].includes(objType)) {
    if (objType === "list" && searchFunc in obj) { return ([key]: string[] | number[]) => key === searchFunc }
    if (funcType === "regexp") {
