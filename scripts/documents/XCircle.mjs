 ...
 */
  ...
  static PING({x, y}, {context, radius = 20, color = "yellow"} = {}) {
    ...
    const [pingElem] = $(`<svg height="100%" width="100%"><circle cx="${radius}" cy="${radius}" r="${radius}" fill="${color}" stroke="none" /></svg>`)
      .appendTo(RollCircle.DBCONTAINER)
      .children()
      .last();
    if (context) {
      ({x, y} = MotionPathPlugin.convertCoordinates(context, RollCircle.DBCONTAINER, {x, y}));
    }
  ...
  get ping() { return RollCircle.PING } 
        ...
        if (prevTime !== timestamp) {
          circle.reaitemdDice.forEach((item) => {
            if (!circle._dbDiceArms[item.name]) {
              $(`<svg id="${item.name}-FRAME" class="diceArmFrame" height="100%" width="100%">
              <line id="${item.name}-ARM" class="diceArm" fill="none" stroke="blue" stroke-width="2" x1="${circle.x}" y1="${circle.y}" x2="${item.x}" y2="${item.y}"></line>
          ...
          circle._dbData = {
            col1: {
              ...
              "dAng": circle.reaitemdDice?.[0] ? circle._getAbsAngleTo(circle.reaitemdDice?.[0]) : 0,
              "dSlot": circle.reaitemdDice?.[0] ? circle._getItemSlot(circle.reaitemdDice?.[0]) : 0,
              "∆Ang": circle.reaitemdDice?.[0] ? U.getAngleDelta(circle.rotation, circle._getAbsAngleTo(circle.reaitemdDice?.[0])) : 0
            },
            col2: {
              cPos: `${U.pad(parseInt(circle.x), 5)}, ${U.pad(parseInt(circle.y), 5)}`,
              dPos: `${U.pad(parseInt(circle.reaitemdDice?.[0]?.x ?? 0), 5)}, ${U.pad(parseInt(circle.reaitemdDice?.[0]?.y ?? 0), 5)}`/* ,
              sPos: `${U.pad(parseInt(circle.snap.points?.[0]?.x), 5)}, ${U.pad(parseInt(circle.snap.points?.[0]?.y), 5)}` */
            },
            col3: {
              "cAng": circle.rotation,
              "dAng": circle.reaitemdDice?.[0] ? circle._getRelAngleTo(circle.reaitemdDice?.[0]) : 0,
              "dSlot": circle.reaitemdDice?.[0] ? circle._getItemSlot(circle.reaitemdDice?.[0]) : 0,
              "∆Ang": circle.reaitemdDice?.[0] ? U.getAngleDelta(circle.rotation, circle._getRelAngleTo(circle.reaitemdDice?.[0])) : 0
            }/* ,
            col3: {
